            ...
            <form id="ticketForm" action="{{ route('vendor.invoice.store') }}" method="POST" class="row" enctype="multipart/form-data">
                ...
                <div class="row clearfix w-100">
                    <div class="col-md-12 text-right">
                        ...
                        <button type="button" class="btn btn-primary px-4 me-2 my-1 m-2 add-multiple-item" data-toggle="modal" data-target="#items-for-invoice"><i class="fas fa-plus"></i> Add Multiple Item</button>
...
<div class="modal fade" id="items-for-invoice" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Add Items into Invoice</h5>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    @if(!empty($vendorMenu))
                    @foreach($vendorMenu as $k=>$v)
                    <div class="col-lg-4 mb-2 item-id-{{ $v->id }}">
                        <div class="card lift lift-sm h-70 single-item" data-name="{{ $v->name }}" data-id="{{ $v->id }}" data-price-half="{{ $v->price_half }}" data-price-full="{{ $v->price_full }}">
                            <div class="card-body">
            ...
            <div class="modal-footer">
                <button class="btn btn-success" type="submit" id="add-selected-items-to-invoice"><i class="bi bi-floppy"></i> Add Selected Items</button>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".delete-item", function () {
            let id = $(this).parents(".item-row").find(".item-id").val();
            $(this).parents(".item-row").remove();
            calc();

            $(".item-id-"+id).removeClass("d-none");
        });
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", "#add-selected-items-to-invoice", function(){
            ...
            $(".selected-item").each(function(){
                $(this).removeClass("selected-item").parent().addClass("d-none");

                let id = $(this).data("id");
                let name = $(this).data("name");
                let price_half = $(this).data("price-half");
                let price_full = $(this).data("price-full");
                ...
                html += '<tr class="item-row">';
                    ...
                    html += '<td>\
                                <div class="item-parent-div">\
                                    <input type="text" name="item[]" placeholder="Enter Item Name" class="form-control search-box" value="'+name+'" />\
                                    <ul class="dropdown-menu"></ul>\
                                </div>\
                                <input type="hidden" name="item_id[]" class="item-id" value="'+id+'" />\
                                <input type="hidden" class="half-price" value="'+price_half+'" />\
                                <input type="hidden" class="full-price" value="'+price_full+'" />\
                            </td>';
                    html += '<td>\
                                <div class="input-group">\
                                    <button type="button" class="btn btn-sm btn-danger me-2 minus"><i class="fas fa-minus"></i></button>\
                                    <input type="text" name="qty[]" inputmode="decimal" placeholder="Enter Qty" class="form-control qty text-center" value="1" min="0" step="0.01">\
                                    <button type="button" class="btn btn-sm btn-primary me-2 plus"><i class="fas fa-plus"></i></button>\
                                </div>\
                            </td>';
                    html += '<td>';
                        html += '<select name="type[]" class="form-control price-type">';
                            html += '<option value="1" selected>Half</option>';
                            html += '<option value="2">Full</option>';
                        html += '</select>';
                    html += '</td>';
                    html += '<td><input type="text" name="price[]" inputmode="decimal" placeholder="Enter Price" class="form-control price decimal-number" step="0.00" min="0" value="'+price_half+'" /></td>';
                    html += '<td><input type="text" name="total[]" placeholder="0.00" class="form-control total" readonly/></td>';
                    html += '<td>';
                        html += '<button type="button" class="btn btn-sm btn-primary m-1 me-2 my-1 add-item add-to-next"><i class="fas fa-plus"></i></button>';
                        html += '<button type="button" class="btn btn-sm btn-danger me-2 my-1 delete-item"><i class="fas fa-trash"></i></button>';
